...
public class GlUtil {
    ...
    public static void logVersionInfo() {
        LogUtils.e("vendor  : " + GLES20.glGetString(GLES20.GL_VENDOR));
        LogUtils.e("renderer: " + GLES20.glGetString(GLES20.GL_RENDERER));
        LogUtils.e("version : " + GLES20.glGetString(GLES20.GL_VERSION));
     ...
     * 创建FBO
     *
     * @return int[]: frameBufferID, renderBufferID, textureColorBufferID
     */
    public static int[] createFBO(int width, int height) {
        int[] result = new int[3];
        int[] frameBuffer = new int[1];
        int[] rbo = new int[1];
        int[] textureColorBuffer = new int[1];
     ...
     */
    public static void releaseFBOBuffer() {
    ...
    /**
     * 释放FBO
     * 注意：在onDrawFrame执行完后，要做过程中所有id的释放工作
     * ids 为createFBO中获取的result
     */
    public static void releaseFBO(int[] ids) {
        GLES20.glDeleteTextures(1, ids, 0); //先删除
        GLES20.glDeleteRenderbuffers(1, ids, 1);
        GLES20.glDeleteFramebuffers(1, ids, 2);
    }
}
